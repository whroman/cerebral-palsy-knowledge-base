# Cerebral Palsy Knowledge Base

A comprehensive, evidence-based knowledge base about cerebral palsy assessment and management, with a particular focus on adults.

## 🎯 Key Features

- **100% Citation-Based**: Every medical claim is backed by legitimate sources
- **Zero AI-Generated Content**: All information comes from peer-reviewed research, clinical guidelines, and official documentation
- **Focus on Adults**: Specifically addresses assessment needs for adults with CP, including those diagnosed later in life
- **Modern Web Interface**: Built with Docusaurus for easy navigation and excellent UX

## 📚 Content Includes

### Classification Systems
- GMFCS (Gross Motor Function Classification System)
- MACS (Manual Ability Classification System)
- CFCS (Communication Function Classification System)

### Assessment Standards
- Comprehensive assessment overview based on NICE guidelines
- Cognitive and intellectual assessment
- Functional independence measures (FIM)

### Resources for Adults
- Late diagnosis information
- Assessment resources for adults who missed childhood evaluation

## 🚀 Quick Start

### Development

```bash
# Install dependencies
npm install

# Start development server
npm start
```

The site will be available at `http://localhost:3000`

### Build for Production

```bash
npm run build
```

This generates static files in the `build` directory.

### Deploy to GitHub Pages

1. Update `docusaurus.config.js` with your GitHub repository information:
   - Set `url` to your GitHub Pages URL
   - Set `baseUrl` to your repository name (e.g., `/cerebral-palsy-kb/`)
   - Set `organizationName` to your GitHub username
   - Set `projectName` to your repository name

2. Deploy:
```bash
npm run deploy
```

## 📖 Sources

All information is sourced from:
- **CanChild McMaster University** - GMFCS official documentation
- **MACS.nu** - Manual Ability Classification System
- **CFCS.us** - Communication Function Classification System
- **NICE Guidelines** - UK clinical guidelines for CP
- **PubMed** - Peer-reviewed research papers
- **Systematic reviews and meta-analyses** from 2020-2024

## ⚖️ License

This knowledge base aggregates information from public medical sources. All original sources retain their respective copyrights.

## ⚠️ Disclaimer

This knowledge base is for educational purposes only. Always consult with qualified healthcare professionals for medical advice and assessment.

## 🔍 Verification

Every claim in this knowledge base can be verified by checking the cited sources. No unsourced medical information is included.